...
function upgrade_set(id, cost, title, desc, eff="none") {
    ...
    document.getElementById("upg").innerHTML =
        document.getElementById("upg").innerHTML + `<button :style="{
        backgroundColor: player.upgrades.indexOf(${id})!=-1?'#99ff99':(upgrade_costs_real[${id}]>level?'#999999':'#ffffff')}" style="position: absolute; left: ${L * 20 % 100}%; top: ${Math.floor(L / 5) * 32}%; width: 18%; height: 30%" v-if='player.upgrades.indexOf(${id - 1})!=-1||${id == 1}' onclick="buy_upgrade(${id})">
...
upgrade_set(14, 700, "I am understanding bigger numbers better!", "Current number counted's effect's softcap is delayed based on written numbers.",
    function () { return player.write.add(1).pow(0.5).pow_base(5) }
)
upgrade_set(15, 740, "And writing numbers more neatly!", "Writing scaling is further decreased by current level.",
